...
export const TaskItemWithTrack: React.FC<TaskItemWithTrackProps> = ({
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!value) return;
    const plannedDays = Array.isArray(task.plannedDays) ? [...task.plannedDays] : [];
  ...
  const handleRemovePlannedDay = (dateStr: string) => {
    const plannedDays = Array.isArray(task.plannedDays) ? [...task.plannedDays] : [];
    onUpdate(path, task.id, { plannedDays: plannedDays.filter(d => d !== dateStr) });
  };
  ...
  return (
    <div className={styles.taskItem}>
      ...
      {Array.isArray(task.plannedDays) && task.plannedDays.length > 0 && (
        <ul className={styles.execList} style={{ marginTop: 4, background: '#eef' }}>
          {task.plannedDays.sort().map((dateStr) => (
            <li key={dateStr} style={{ display: 'flex', alignItems: 'center' }}>
              <span style={{ fontSize: 13, color: '#336' }}>Planeado: {dateStr}</span>
              <button
                style={{ marginLeft: 8, color: '#c00', background: 'none', border: 'none', cursor: 'pointer', fontSize: 14 }}
